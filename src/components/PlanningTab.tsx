...
const PlanningTab = ({ onOpenDetail, tripData, onUpdateCoverImage }: PlanningTabProps) => {
  ...
  return (
    <div className="planning-layout flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-8 lg:gap-10" style={{ marginTop: '24px' }}>
      {/* Arched Cover Image or Camera Placeholder */}
      <div className="planning-arch-photo flex-shrink-0 w-full max-w-[280px] md:min-w-[340px] md:max-w-[360px] lg:min-w-[420px] lg:max-w-[440px]">
        <input
          ref={fileInputRef}
          type="file"
          accept="image/*"
      ...
      <div className="flex-1 flex flex-col justify-end md:justify-start items-center md:items-start gap-4 sm:gap-5 lg:gap-3 pr-4 md:pr-6 overflow-visible">
        {/* Headline */}
        <motion.div
        ...
        >
          <span className="font-script text-3xl sm:text-4xl lg:text-[3.4rem] my-0 py-[15px]" style={{ color: 'hsl(0 20% 32%)' }}>
            happily ever after
          </span>
          <span
            className="font-serif text-xl sm:text-2xl lg:text-[2.4rem] text-foreground/50 ml-2 inline py-[15px] mx-px"
