      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 left-0 bottom-0 w-[280px] bg-background z-50 px-8 py-16"
          >
            ...
            <nav className="space-y-2">
              {menuItems.map((item) => (
                <button
                  ...
                  className="w-full flex items-center gap-4 px-4 py-3 pill-shape text-foreground/70 hover:bg-primary/30 hover:text-foreground transition-colors"
                >
            ...
            <div className="absolute bottom-12 left-8 right-8 flex gap-3 justify-center">
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  className="w-16 h-20 bg-primary/40 arch-shape"
                />
              ))}
            </div>
          </motion.div>
